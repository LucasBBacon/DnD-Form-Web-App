...
describe("DeathSavesTracker", () => {
  it("renders the section header", () => {
    render(
      <DeathSavesTracker success={0} failure={0} onToggle={() => {}} />,
    );
  ...
  it("checks the correct number of success boxes", () => {
    const { container } = render(
      <DeathSavesTracker success={2} failure={0} onToggle={() => {}} />,
    );
    const successCheckboxes = container.querySelectorAll(
  ...
  it("calls onToggle with 'successes' when a success box is changed", async () => {
    ...
    const { container } = render(
      <DeathSavesTracker success={0} failure={0} onToggle={onToggle} />,
    );
