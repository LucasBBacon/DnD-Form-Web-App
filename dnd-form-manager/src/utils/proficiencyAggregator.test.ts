...
import { aggregateNonSkillProficiencies, aggregateProficiencies, aggregateSaveProficiencies, aggregateSkillProficiencies } from "./proficiencyAggregator";
...
describe("proficiencyAggregator", () => {
    ...
    it("normalizes non-skill proficiency identifiers into stable slices", () => {
        ...
        expect(result.weapons.list).toEqual(["simple", "weapon_longsword", "martial"]);
        expect(result.armor.list).toEqual(["shield"]);
        expect(result.tools.list).toEqual(["tool_thieves_tools", "category_artisans_tools"]);
